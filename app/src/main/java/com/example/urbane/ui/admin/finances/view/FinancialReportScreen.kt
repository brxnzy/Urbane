...
fun FinancialReportScreen(viewModel: FinancesViewModel) {
    ...
    // Manejar el éxito de la generación del PDF
    LaunchedEffect(state.success) {
        when (val success = state.success) {
            is FinancesSuccess.PDFGenerated -> {
                val result = snackbarHostState.showSnackbar(
                    message = "${success.fileName} guardado en Descargas",
                    actionLabel = "Abrir",
                ...
                if (result == SnackbarResult.ActionPerformed) {
                    try {
                        val intent = Intent(Intent.ACTION_VIEW).apply {
                            setDataAndType(success.uri, "application/pdf")
                        ...
                        snackbarHostState.showSnackbar(
                            message = "No se encontró una aplicación para abrir PDF",
    ...
    Scaffold(
        ...
        Column(
                ...
                OutlinedCard(
                    ...
                    Column(
                        ...
                        Row(
                            ...
                            Spacer(modifier = Modifier.width(8.dp))
                            Text(
                                text = formatDate(state.endDate) ?: "Seleccionar",
                                style = MaterialTheme.typography.bodyLarge
                            )
                        }
                    }
            ...
            if (state.startDate != null && state.endDate != null) {
                ...
                Button(
                    onClick = { viewModel.handleIntent(FinancesIntent.GeneratePDF(context)) },
                    modifier = Modifier
                        .fillMaxWidth()
                        .height(56.dp),
    ...
    if (showStartDatePicker) {
        DatePickerModal(
            onDateSelected = { date ->
                viewModel.handleIntent(FinancesIntent.UpdateStartDate(date))
